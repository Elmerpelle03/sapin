...
require '../../config/session_superadmin.php'; // Only superadmin can view admins
...
$columns = ['user_id', 'username', 'fullname', 'email', 'is_verified', 'accountstatus_name'];
...
$sql = "SELECT 
        ...
        LEFT JOIN accountstatus ac ON u.accountstatus_id = ac.accountstatus_id
        ...
        LEFT JOIN usertype ut ON u.usertype_id = ut.usertype_id
        WHERE 
            u.usertype_id = 1 AND (
...
$total_stmt = $pdo->query("SELECT COUNT(*) FROM users WHERE usertype_id = 1");
$total_records = $total_stmt->fetchColumn();

$filtered_stmt = $pdo->prepare("SELECT COUNT(*) FROM users u JOIN userdetails ud ON u.user_id = ud.user_id WHERE u.usertype_id = 1 AND (u.username LIKE :search1 OR CONCAT(ud.firstname, ' ', ud.lastname) LIKE :search2 OR u.email LIKE :search3)");
