...
<head>
	...
	<style>
		...
		.stats-card.verified { border-left-color: #10b981; }
		.stats-card.unverified { border-left-color: #f59e0b; }
			...
			<main class="content">
				<div class="container-fluid p-0">
					...
					<div class="page-header">
						...
						<p class="mb-0 mt-2" style="opacity: 0.9;">Manage customer accounts and user information</p>
					</div>

					<?php
					// Fetch user statistics
					$total_users = $pdo->query("SELECT COUNT(*) FROM users WHERE usertype_id = 2")->fetchColumn();
					$verified_users = $pdo->query("SELECT COUNT(*) FROM users WHERE usertype_id = 2 AND is_verified = 1")->fetchColumn();
					$unverified_users = $pdo->query("SELECT COUNT(*) FROM users WHERE usertype_id = 2 AND is_verified = 0")->fetchColumn();
					?>
